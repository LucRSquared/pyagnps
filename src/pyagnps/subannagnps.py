...
def build_network(df):
    ...
    G = nx.from_pandas_edgelist(df,source='Reach_ID', 
...
def subset_and_rename_reach_df(df, relabel_dict):
    ...
    df_sub = df.loc[df['Reach_ID'].isin(old_reaches),:].reset_index(drop=True).copy(deep=True)

    df_sub['Reach_ID'] = df_sub['Reach_ID'].apply(lambda x: relabel_dict[x])
    df_sub['Receiving_Reach'] = df_sub['Receiving_Reach'].apply(lambda x: rename_reach(relabel_dict,x))
...
def subset_and_rename_cell_df(df, relabel_dict):
    ...
    df_sub['Reach_ID'] = df_sub['Reach_ID'].apply(lambda x: rename_reach(relabel_dict,x))

    return df_sub

def subset_and_rename_asc_file(labels, path_to_in_asc, path_to_out_asc, keep_tmp_gtiff=False):
    ...
    if type(labels) is list:
        out_data = np.where(np.isin(in_data, labels), in_data, nodata) # Only keep valid rasters
    elif type(labels) is dict:
        original_values = list(labels.keys())
        out_data = np.where(np.isin(in_data, original_values), in_data, nodata) # Only keep valid rasters and relabel them
        unique_reaches, inv = np.unique(out_data, return_inverse=True)
        out_data = np.array([labels.get(old_reach, nodata) for old_reach in unique_reaches])[inv].reshape(out_data.shape)
    ...
    # Convert to ASC
    opts = gdal.WarpOptions(format="AAIGrid", srcNodata=nodata, dstNodata=nodata, xRes=x_res, yRes=y_res, resampleAlg=gdal.GRA_NearestNeighbour)
...
def subset_and_rename_cell_IDs_asc(df_cells_sub, path_to_cell_ids_asc, outpath_to_cell_ids_asc, keep_tmp_gtiff=False):

    valid_cells = df_cells_sub['Cell_ID'].to_list()

    subset_and_rename_asc_file(valid_cells, path_to_cell_ids_asc, outpath_to_cell_ids_asc, keep_tmp_gtiff=keep_tmp_gtiff)

    return

def subset_and_rename_reach_IDs_asc(relabel_dict, path_to_reach_ids_asc, outpath_to_reach_ids_asc, keep_tmp_gtiff=False):
    ...
    subset_and_rename_asc_file(relabel_dict, path_to_reach_ids_asc, outpath_to_reach_ids_asc, keep_tmp_gtiff=keep_tmp_gtiff)
