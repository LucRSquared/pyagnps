...
path_to_log_dir = f'{root_dir}/LOGS/' ; os.makedirs(path_to_log_dir)
path_to_qc_dir = f'{root_dir}/QualityControl/' ; os.makedirs(path_to_qc_dir)
...
for _, tuc in thucs.iterrows():
    ...
    try:
        ...
        now = get_current_time()
        log_to_file(path_to_general_log, f'{now}: {thuc_id}: Deleted {len(file_del_errors)} unnecessary files...')
