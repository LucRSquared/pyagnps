...
outpath_cells = Path(
    "/aims-nas/luc/thuc_soil_survey/cells_NITA_unvalid_soils_reason_gNATSGO.gpkg"
)
...
soil_root_path = Path(
    "/aims-nas/data/datasets/SDM_QUERY_AND_NITA_PROCESSING/ALL_US_v2_SSURGO_STATSGO2_RSS/"
)
...
path_to_3soil_sources_rvindicator_yes = (
    soil_root_path / "raw_query_data" / "soil_data_ALL_SOURCES_COMBINED_with_mukey.csv"
)
path_to_3soil_sources_rvindicator_no_and_NULL = (
    soil_root_path / "raw_query_data" / "ALL_SOURCES_rvindicator_no_and_NULL.csv"
)

df_raw_all = pd.concat(
    [
        pd.read_csv(path_to_3soil_sources_rvindicator_yes),
...
for thuc_id in tqdm(runlist):
    ...
    try:
        with engine.connect() as conn:
            ...
            cell_data_section = cell_data_section[
                ~cell_data_section["soil_id"].isin(valid_soil_ids)
            ]  # the row that does the job

            populated_invalid_cells = cell_data_section["cell_id"].to_list()

            if populated_invalid_cells:
                query_geom = "SELECT dn, geom FROM thuc_{}_annagnps_cell_ids WHERE dn in ({})".format(
                    thuc_id, ",".join(str(x) for x in populated_invalid_cells)
                )

                cells = gpd.read_postgis(
                    sql=sql_text(query_geom), con=conn, geom_col="geom"
                )
                cells = cells.to_crs("epsg:4326")
                cells["thuc"] = thuc_id

                invalid_rows = cells.merge(
                    cell_data_section, left_on="dn", right_on="cell_id"
                )
                ...
                invalid_rows = invalid_rows.merge(
                    df_nita_discarded, how="left", on="soil_id"
                )

                # Add companame (soil description)
                invalid_rows = invalid_rows.merge(
                    df_soil_id_compname, how="left", on="soil_id"
                )
                ...
                if category in invalid_rows["problems"].cat.categories:
                    invalid_rows.loc[
                        invalid_rows["problems"].isna(), "problems"
                    ] = category
                else:
                    # Add the new category to the existing categories
                    invalid_rows["problems"] = invalid_rows[
                        "problems"
                    ].cat.add_categories(category)
                    invalid_rows.loc[
                        invalid_rows["problems"].isna(), "problems"
                    ] = category
...
invalid_cells = gpd.GeoDataFrame(
    pd.concat(rows_invalid_soil_id, ignore_index=True), crs="epsg:4326", geometry="geom"
)
