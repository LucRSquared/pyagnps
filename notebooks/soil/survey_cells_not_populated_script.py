...
outpath_cells = Path(
    "../../outputs/soil_data_market/db_population/cells_without_soil_ssurgo.gpkg"
)
    ...
    try:
        with engine.connect() as conn:
            ...
            if unpopulated_cells:
                query_geom = (
                    "SELECT * FROM thuc_{}_annagnps_cell_ids WHERE dn in ({})".format(
                        thuc_id, ",".join(str(x) for x in unpopulated_cells)
                    )
                )
...
empty_cells = gpd.GeoDataFrame(
    pd.concat(cells_without_soil, ignore_index=True), crs="epsg:4326", geometry="geom"
)
