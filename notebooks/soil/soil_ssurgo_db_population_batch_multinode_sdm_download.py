...
for _, tuc in thucs.iterrows():
    ...
    if goodsofar:
        try:
            ...
            if thuc_ssurgo_file.exists():
                log_to_file(
                    general_log,
                    f"{now}: {nodename}: {thuc_id}: SSURGO file already exists, using it now",
                )

            else:
                log_to_file(
                    general_log,
                    f"{now}: {nodename}: {thuc_id}: Downloading SSURGO data",
                )
                bbox = cells.envelope.buffer(BUFFER).to_crs(epsg=4326).total_bounds
                geo_soil = sdm.download_soil_geodataframe_tiles(
                    bbox, tile_size=TILE_SIZE
                )
    ...
    if goodsofar:
        try:
            ...
            if thuc_ssurgo_file.exists():
                log_to_file(
                    general_log, f"{now}: {nodename}: {thuc_id}: Reading SSURGO file"
                )
                geo_soil = gpd.read_file(thuc_ssurgo_file)

            else:
                log_to_file(
                    general_log, f"{now}: {nodename}: {thuc_id}: Saving SSURGO file"
                )
