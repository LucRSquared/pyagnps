...
keep_files_failure = [
    ...
    "TopAGNPS_log.CSV",
    "TopAGNPS_status.CSV",
    "TopAGNPS_wrn.CSV" "TopAGNPS_err.CSV",
    "TOPAGNPS.XML",
]
...
for _, tuc in thucs.iterrows():
    ...
    if not (
        os.path.exists(f"{root_dir}/{dir_runs_name}")
        and os.path.isdir(f"{root_dir}/{dir_runs_name}")
    ):
    ...
    if os.path.exists(path_to_dir) and os.path.isdir(path_to_dir):
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: THUC previously computed: SKIPPING",
        )
        continue
    else:
        topagnps.create_topagnps_directory(
            f"{root_dir}/{dir_runs_name}", thucid_dir_name
        )
    ...
    try:
        now = get_current_time()
        log_to_file(
            path_to_general_log, f"{now}: {nodename}: {thuc_id}: Downloading DEM"
        )
        dem, path_to_asc = topagnps.download_dem(
            thuc_select,
            path_to_run_dir,
            name=thucid_dir_name,
            resolution_m=RESOLUTION,
            buffer_m=BUFFER,
        )

        dem_filename = path_to_asc.rsplit("/", 1)[
            -1
        ]  # Part of the string after the last / = "thuc_1173_rest_10_m.asc"
        print(dem_filename)
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Creating TopAGNPS control file",
        )
        ...
        now = get_current_time()
        log_to_file(
            path_to_general_log, f"{now}: {nodename}: {thuc_id}: Failed to download DEM"
        )
        # Delete path_to_dir so that it can be reattempted some other time
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Deleting run and output directory to reattempt later",
        )
    ...
    try:
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: TopAGNPS pre-processing step",
        )

        # topagnps.run_topagnps(path_to_run_dir, path_to_TOPAGNPS_bin)
        topagnps.run_topagnps(
            path_to_run_dir,
            path_to_TOPAGNPS_bin,
            memtrack=True,
            output_memtrack_filename="valgrind_topagnps_preprocessing.txt",
        )
    except:
        now = get_current_time()
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Error! Failed to run pre-processing",
        )
    ...
    try:
        now = get_current_time()
        log_to_file(
            path_to_general_log, f"{now}: {nodename}: {thuc_id}: Finding outlet..."
        )
        (
            xout,
            yout,
            ...
            colout,
            raster_crs,
        ) = topagnps.find_outlet_uparea_shape_intersection(
            path_to_run_dir + "/UPAREA.ASC", thuc_select
        )

        now = get_current_time()
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Outlet found! x = {xout}, y = {yout} ({raster_crs}) / row = {rowout}, col = {colout}",
        )
    except:
        now = get_current_time()
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Error! Failed to find outlet",
        )
        log_to_file(path_to_time_log, f"{thuc_id},0")
        log_to_file(path_to_thuc_faillist, f"{thuc_id}")
        # continue
    ...
    try:
        ...
        shutil.copy2(
            f"{path_to_run_dir}/RELIEF.ASC", f"{path_to_run_dir}/{new_dem_name}"
        )
        shutil.copy2(
            f"{path_to_run_dir}/RELIEF.PRJ", f"{path_to_run_dir}/{new_prj_name}"
        )
        ...
        now = get_current_time()
        log_to_file(
            path_to_general_log, f"{now}: {nodename}: {thuc_id}: Updating control file"
        )
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Finishing TopAGNPS processing with outlet information",
        )
        ...
        topagnps.run_topagnps(
            ...
            path_to_TOPAGNPS_bin,
            memtrack=True,
            output_memtrack_filename="valgrind_topagnps_full_processing.txt",
        )
    except:
        now = get_current_time()
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Error! Failed to finish TopAGNPS processing",
        )
        log_to_file(path_to_time_log, f"{thuc_id},0")
        log_to_file(path_to_thuc_faillist, f"{thuc_id}")
        # continue
    ...
    try:
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Computing quality control for THUC",
        )
        ...
        log_to_file(
            f"{path_to_qc_dir}/{thuc_id}_res_{RESOLUTION}_csa_{CSA}_mscl_{MSCL}.json",
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Error! Failed to compute quality control",
        )
        log_to_file(path_to_time_log, f"{thuc_id},0")
        log_to_file(path_to_thuc_faillist, f"{thuc_id}")
        # continue
    ...
    if everythingwentwell:
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Deleting unnecessary files...",
        )
        ...
        log_to_file(
            path_to_general_log,
            f"{now}: {nodename}: {thuc_id}: Finished normally in {end-start} seconds",
        )
        ...
        if path_to_dir != path_to_run_dir:
            ...
            if len(move_files_errors) == 0:
                ...
                log_to_file(
                    path_to_general_log,
                    f"{now}: {nodename}: {thuc_id}: Files moved successfully!",
                )
                # Remove run directory
                log_to_file(
                    path_to_general_log,
                    f"{now}: {nodename}: {thuc_id}: Removing run directory...",
                )
                shutil.rmtree(path_to_run_dir)
            else:
                now = get_current_time()
        ...
        file_del_errors = remove_all_files_from_dir_except_from_list(
            path_to_run_dir, keep_files_failure_tmp
        )
        ...
        if path_to_dir != path_to_run_dir:
            ...
            log_to_file(
                path_to_general_log,
                f"{now}: {nodename}: {thuc_id}: Moving log files of failed delineation to output directory...",
            )
            move_files_errors = move_files_from_dir_to_dir(path_to_run_dir, path_to_dir)
            ...
            if len(move_files_errors) == 0:
                ...
                log_to_file(
                    path_to_general_log,
                    f"{now}: {nodename}: {thuc_id}: Log files moved successfully!",
                )
                # Remove run directory
                log_to_file(
                    path_to_general_log,
                ...
                log_to_file(
                    path_to_general_log,
                    f"{now}: {nodename}: {thuc_id}: Error! Failed to move log files from {path_to_run_dir} to {path_to_dir}",
                )
